...
from func import cudnn_gru, native_gru, dot_attention, summ, dropout, ptr_net, Relation_Module
...
class Model(object):
    ...
    def ready(self):
        ...
        with tf.variable_scope("relation analysis"):
            ...
            md=Relation_Module(config,self.c_maxlen,self.q_maxlen, g_theta_layers)
            #r add attention weight with q_summary
            r,alpha=md.hop_2(c,init,phase=self.is_train,activation=tf.nn.relu)
            c=r[-1]
