...
def train(config):
    ...
    patience = 0
    lr = config.init_lr
    #lr=tf.constant(config.init_lr,dtype=tf.float32)
        ...
        for _ in tqdm(range(1, config.num_steps + 1)):
            ...
            if global_step % config.checkpoint == 0:
                ...
                if patience >= config.patience:
                    ...
                    loss_save = dev_loss
                    patience = 0
                lr_sum=tf.Summary(value=[tf.Summary.Value(tag="model/lr",simple_value=lr),])
                graph_handler.add_summary(lr_sum,global_step)
                sess.run(tf.assign(model.lr, tf.constant(lr, dtype=tf.float32)))
                graph_handler.add_summaries(summ, global_step)
    ...
    with tf.Session(config=sess_config) as sess:
        ...
        for step in tqdm(range(total // config.batch_size + 1)):
            start_logits,stop_logits,qa_id, loss, yp1, yp2 = sess.run(
                [model.start_logits,model.stop_logits,model.qa_id, model.loss, model.yp1, model.yp2])
